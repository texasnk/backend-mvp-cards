...
- Persistir automaticamente os cards gerados.
...
| RF-10 | O sistema deve exigir que cada card possua os campos `frente` e `verso`, com mínimo de 3 caracteres em cada campo. | Obrigatório | US-02, US-03, CA-02, CA-03 |
...
| RF-22 | O sistema deve persistir automaticamente os cards gerados com os campos `frente`, `verso`, abordagem e domínio associado. | Obrigatório | US-03, CA-03, CA-06 |
...
| RF-29 | O sistema deve rejeitar requisições de geração de cards sem conteúdo utilizável após extração/OCR. | Obrigatório | US-03, CA-03 |
| RF-30 | Quando um PDF não permitir extração textual direta por se tratar de imagem, o sistema deve enviar o conteúdo à OpenAI e aguardar o retorno do texto para continuidade do processamento. | Obrigatório | US-03, CA-03 |
...
| RNF-14 | O sistema deve ser estruturado para permitir evolução futura sem acoplamento explícito a tecnologia de OCR, banco ou provedor de IA. | Obrigatório | |
| RNF-15 | A resolução máxima aceita para imagens enviadas ao processamento deve ser 720p. | Obrigatório | |
| RNF-16 | Em caso de erro, a API deve retornar status code compatível com a falha e mensagem objetiva, sem incluir informação sensível. | Obrigatório | |
...
| RB-04 | O conteúdo mínimo obrigatório de um card é `frente` e `verso`, com pelo menos 3 caracteres em cada campo. | RF-10 |
...
| RB-12 | A sugestão de domínio deve obedecer às mesmas regras de criação de domínio, com nome entre 3 e 40 caracteres. | RF-27 |
| RB-13 | Resumo geral e cards só podem ser gerados a partir de conteúdo textual efetivamente extraído; falhas de extração devem retornar exceção. | RF-18, RF-20, RF-29, RF-30 |
| RB-14 | O resumo geral não deve ser persistido. | RF-19 |
| RB-15 | Cards gerados no processo automático devem permanecer consistentes com o conteúdo de origem enviado para a API da OpenAI. | RF-17 a RF-22 |
| RB-16 | O processamento deve ocorrer em PT-BR. | RF-18, RF-21, RF-26, RNF-13 |
...
| US-03 | Como usuário integrador do backend, quero enviar PDF, imagem ou texto para obter resumo e cards automaticamente, informando opcionalmente um domínio. | RF-11 a RF-23, RF-29, RF-30 |
| US-04 | Como usuário integrador do backend, quero que o sistema identifique o domínio mais aderente ou sugira um novo quando eu não informar domínio. | RF-24 a RF-28 |
...
| CA-02 | Dado um domínio existente, quando o cliente criar, consultar, atualizar ou excluir um card manual com `frente` e `verso` contendo no mínimo 3 caracteres cada, então a API deve persistir e recuperar os dados corretamente. | US-02 |
| CA-03 | Dado um PDF, imagem ou texto puro com conteúdo utilizável, quando o cliente solicitar processamento e informar quantidade de cards válida, então o sistema deve extrair texto, gerar resumo geral, gerar a quantidade solicitada de cards, classificar cada card em uma abordagem permitida e persisti-los com domínio associado. | US-03 |
| CA-04 | Dado que o cliente não informou domínio, quando houver aderência com domínio existente, então o sistema deve retornar o domínio identificado e associar os cards gerados a ele; e quando não houver aderência, então deve retornar uma sugestão válida de domínio apenas se a OpenAI a fornecer. | US-04 |
