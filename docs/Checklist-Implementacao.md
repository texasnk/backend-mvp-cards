...
| 5 | Base técnica | Configurar `docker-compose.yml` com `api` e `postgres` | CONCLUÍDO |  | 3, 4 | `docker-compose.yml`, `Dockerfile` | Compose sobe PostgreSQL local e imagem da API com `poppler-utils`. |
...
| Decisão | Adaptadores de PDF usam binários do sistema (`pdftotext`, `pdftoppm` e `pdfinfo`). | Mantém aderência ao documento arquitetural e reduz acoplamento prematuro a bibliotecas específicas. |  | REGISTRADO | O `Dockerfile` já inclui `poppler-utils`. |
