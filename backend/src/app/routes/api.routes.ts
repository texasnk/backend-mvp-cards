...
export function createApiRoutes(
  ...
  const router = Router();
  const processingRateLimit = createRateLimitMiddleware(
    dependencies.processingRateLimit,
  );
  ...
  router.patch(
    "/api/v1/domains/:id",
    wrap(controllers.domainController.update),
  );
  router.delete(
    "/api/v1/domains/bulk",
    wrap(controllers.domainController.deleteMany),
  );
  router.delete(
    "/api/v1/domains/:id",
    wrap(controllers.domainController.delete),
  );

  router.post("/api/v1/cards", wrap(controllers.cardController.create));
  router.post(
    "/api/v1/cards/import",
    wrap(controllers.cardController.importBlock),
  );
  router.delete(
    "/api/v1/cards/bulk",
    wrap(controllers.cardController.deleteMany),
  );
  ...
  router.get(
    "/api/v1/cards/:id/review-options",
    wrap(controllers.reviewController.options),
  );
  router.post(
    "/api/v1/cards/:id/reviews",
    wrap(controllers.reviewController.answer),
  );
  ...
  router.get("/api/v1/metrics", (_request, response) => {
    response
      .status(200)
      .type("text/plain")
      .send(dependencies.metrics.renderPrometheus());
  });
